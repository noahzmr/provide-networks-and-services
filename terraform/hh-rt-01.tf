...
resource "ciscoios_interface" "gigabitethernet0/0/0" {
  ...
  ipv6_address = "2001:DB8:2000:2::1/64"
  vlan_mapping {
    vlan_id = 2
    dot1q   = true
  }
...
resource "ciscoios_interface" "gigabitethernet0/0/0.3" {
  ...
  ip_subnet           = 26
  ip_helper_address   = "10.0.3.3"
  ipv6_address        = "2001:DB8:2000:3::1/64"
}
...
resource "ciscoios_line_vty" "line_vty0" {
  transport_input = "ssh"
  login_local     = true
}

resource "ciscoios_line_vty" "line_vty1" {
  ipv6_access_class = "HH-SSH"
  login_local       = true
  transport_input   = "ssh"
}