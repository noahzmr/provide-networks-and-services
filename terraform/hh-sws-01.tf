...
resource "ciscoios_interface_vlan" "vlan2" {
  ...
  ip_addresses = ["10.0.1.2 255.255.255.192"]
  ipv6_addresses = ["2001:DB8:2000:2::2/64"]
}

resource "ciscoios_interface_vlan" "vlan3" {
  vlan_id      = 3
  mac_address  = "0007.ec7b.6d01"
  ip_addresses = ["10.0.1.66 255.255.255.192"]
  ipv6_addresses = ["2001:DB8:2000:3::2/64"]
}
